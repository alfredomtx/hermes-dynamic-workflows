...
required; `whenToUse`, `phases`, and `agents` optional). `meta["agents"]` is a pure-literal map of workflow-local role and tool-permission presets. Routing and tool budgets are inline; omitted `maxTurns` resolves from the global `max_turns` config default and explicit inline values override it.

| Global | Signature | Description |
|---|---|---|
| `agent` | `await agent(prompt, opts=None)` | Spawns a subagent. Without a schema it returns text; with `schema` it returns the validated object. Every call requires inline `provider`, canonical `model`, `reasoningEffort`, `maxToolCalls`, and `maxToolOutputChars`; omitted `maxTurns` resolves from plugin config `max_turns` (default 150, clamped to 1..1000), while an explicit inline value overrides it. Malformed or invalid explicit values fail before reservation. Role/tool opts include `agentType`, `instructions`/`systemPrompt`, `toolsets`, `allowedTools`, and `disallowedTools`. Returns `None` if skipped by the user. |
