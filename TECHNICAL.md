...
required; `whenToUse`, `phases`, and `agents` optional). `meta["agents"]` is a pure-literal map of workflow-local role and tool-permission presets. Routing is inline; omitted `maxTurns` resolves from the plugin `max_turns` default and explicit inline values override it. Tool-call and tool-output counts remain observational telemetry only.

| Global | Signature | Description |
|---|---|---|
| `agent` | `await agent(prompt, opts=None)` | Spawns a subagent. Without a schema it returns text; with `schema` it returns the validated object. Every call requires inline `provider`, canonical `model`, and `reasoningEffort`. `maxTurns` is optional; omitted values resolve from plugin config (`max_turns` 150), while explicit inline values override the default. Malformed or invalid explicit values fail before reservation. Tool-call and tool-output counts are observational telemetry only. Role/tool opts include `agentType`, `instructions`/`systemPrompt`, `toolsets`, `allowedTools`, and `disallowedTools`. Returns `None` if skipped by the user. |
| `start_agent` / `wait_agent` / `agent_status` | `handle = await start_agent(prompt, opts=None)` | Starts a durable child turn, waits for it, or reads its persisted lifecycle state. `agent()` is the start-plus-wait shorthand. |
| `continue_agent` / `fork_agent` | `await continue_agent(handle, prompt)` | Continuation reuses the same child session, stable transcript, and workspace. Fork creates a new handle/session from the stable transcript without copying workspace state. Both require an explicit prompt. |
| `stop_agent` | `await stop_agent(handle)` | Requests targeted cooperative interruption. Running handles remain `stopping` until the worker exits; stop never treats asyncio task cancellation as worker termination. |
...
Invalid workflow script: meta.phases.detail must be a string
...
- **Child tool telemetry**: tool-call and tool-output counts are recorded for
  observability and reporting only; they are not child hard limits and do not affect
  resume identity.
...
- **Inline agent opts**: `agent("task", {"provider":"openai-codex", "model":"gpt-5.6-luna", "reasoningEffort":"xhigh", "maxTurns":8, "instructions":"read-only reviewer", "toolsets":["file"], "allowedTools":["read_file", "search_files"]})` supplies the routing contract plus optional role/tool fields. `maxTurns` may be omitted; omitted values resolve from the plugin default, and explicit inline values override it. `toolsets: []` is intentional no tools. `allowedTools: []` denies all normal tools; schema children still keep `structured_output` so they can submit results.
- **Runtime presets**: `meta["agents"]` defines reusable role/tool presets. Example: `meta = {"name":"x", "description":"x", "agents": {"reviewer": {"instructions":"Review only", "toolsets":["file"]}}}`. Each `agent(..., {"agentType":"reviewer", ...})` still declares routing inline; `maxTurns` resolves from the plugin default when omitted.
