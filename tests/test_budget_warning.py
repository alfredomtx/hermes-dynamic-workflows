...
class BudgetWarningTests(unittest.TestCase):
    def test_warns_for_low_literal_turn_limit_on_default_tool_surface(self):
        script = script_for(
            COMMON_OPTIONS.format(max_turns=20) + ', "label": "hunter"'
        )
    ...
    def test_warns_for_low_turn_limit_on_broad_explicit_toolsets(self):
        script = script_for(
    ...
    def test_literal_none_allowed_tools_inherits_default_surface(self):
        script = script_for(
            COMMON_OPTIONS.format(max_turns=20) + ', "allowedTools": None'
        )
