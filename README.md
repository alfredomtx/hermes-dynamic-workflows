...
- `agent(prompt, opts)` spawns a child agent. Each call may declare `provider`, canonical
  `model`, and `reasoningEffort` inline, or omit them to inherit native parent routing.
  Native `profile` may also be passed. Hermes core performs route/profile/provider/model/
  reasoning/fallback resolution. `maxTurns` is optional: omitted values resolve from
  plugin config `max_turns` (150), clamped to 1..1000; explicit inline values override
  the configured default. Malformed or invalid values fail before reservation and launch.
  Presets define role instructions and tool permissions only; they do not supply routing
  or `maxTurns`. Current Bedrock and `codex_app_server` transports do not forward
...
`allowed_tools`, `disallowed_tools`, and `isolation`. Presets do not supply routing or
`maxTurns`; each `agent()` call can declare those options inline, or use native parent
routing and configured defaults when they are omitted.
