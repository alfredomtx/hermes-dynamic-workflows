...
_DESCRIPTION = (
    ...
    '\n'
    '  phase("Scan")\n'
    '  flaky = await agent("grep CI logs for retry markers", {"schema": FLAKY_SCHEMA, "provider": "openai-codex", "model": "gpt-5.6-luna", "reasoningEffort": "high", "maxTurns": 8})\n'
    '  ...\n'
    '\n'
    'The `meta` dict must be a PURE LITERAL — no variables, function calls, dict unpacking, or f-string interpolation. Required fields: `name`, `description`. Optional: `whenToUse` (shown in the workflow list), `phases`, and `agents`. `meta["agents"]` defines workflow-local role presets for stable instructions and tool permissions only; provider, canonical `model`, and reasoning effort may be omitted to inherit native parent routing; native `profile` may be passed, and the core performs route/profile/provider/model/reasoning/fallback resolution. `maxTurns` is optional and resolves from configured plugin `max_turns` (default 150, clamped to 1..1000); no budget is inherited from a role preset. Use the SAME phase titles in meta["phases"] as in phase() calls — titles are matched exactly; a phase() call with no matching meta entry just gets its own progress group.\n'
    '\n'
    'Script body hooks:\n'
    'Child turn budgets are hard ceilings, not telemetry. Always reserve a final model turn for structured output. Count inspect + execute + diagnose + report, not discovery alone.\n'
    'Starting points for broad work are a benchmark/environment loop at 25–40 turns or broad multi-source synthesis at 16–30 turns. Keep benchmark evaluation separate from live probes. If a script has more than 3 phases, split phases through pipeline() or separate workflows. Evidence-only synthesizers should use toolsets=[] and no normal repository tools.\n'
    ...
    '          {"label": "verify:" + f["file"], "phase": "Verify", "schema": VERDICT_SCHEMA, "provider": "openai-codex", "model": "gpt-5.6-luna", "reasoningEffort": "high", "maxTurns": 8},\n'
    '      )\n'
    '      merged = dict(f)\n'
    ...
    '          d["prompt"],\n'
    '          {"label": "review:" + d["key"], "phase": "Review", "schema": FINDINGS_SCHEMA, "provider": "openai-codex", "model": "gpt-5.6-luna", "reasoningEffort": "high", "maxTurns": 8},\n'
    '      ),\n'
    ...
    '      result = await agent("Find bugs in this codebase.", {"schema": BUGS_SCHEMA, "provider": "openai-codex", "model": "gpt-5.6-luna", "reasoningEffort": "high", "maxTurns": 8})\n'
