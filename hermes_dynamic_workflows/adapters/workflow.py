...
_DESCRIPTION = (
    ...
    '  ...\n'
    '\n'
    'The `meta` dict must be a PURE LITERAL — no variables, function calls, dict unpacking, or f-string interpolation. Required fields: `name`, `description`. Optional: `whenToUse` (shown in the workflow list), `phases`, and `agents`. `meta["agents"]` defines workflow-local role presets for stable instructions and tool permissions only; provider, model, reasoning effort, and child budgets belong inline on every `agent()` call. Use the SAME phase titles in meta["phases"] as in phase() calls — titles are matched exactly; a phase() call with no matching meta entry just gets its own progress group.\n'
    '\n'
    'Script body hooks:\n'
