...
def _launch_message(record: dict[str, Any]) -> str:
    ...
    warnings = record.get("budgetWarnings") or []
    if warnings:
        lines.extend(
            [
                "",
                "Advisory child-budget warnings:",
                *[f"- {warning}" for warning in warnings if str(warning).strip()],
            ]
        )
    return "\n".join(lines)
...
_DESCRIPTION = (
    ...
    '\n'
    '  phase("Scan")\n'
    '  flaky = await agent("grep CI logs for retry markers", {"schema": FLAKY_SCHEMA, "provider": "openai-codex", "model": "gpt-5.6-luna", "reasoningEffort": "high", "maxTurns": 8, "maxToolCalls": 16, "maxToolOutputChars": 200000})\n'
    '  ...\n'
    '\n'
    'The `meta` dict must be a PURE LITERAL — no variables, function calls, dict unpacking, or f-string interpolation. Required fields: `name`, `description`. Optional: `whenToUse` (shown in the workflow list), `phases`, and `agents`. `meta["agents"]` defines workflow-local role presets for stable instructions and tool permissions only; provider, canonical `model`, and reasoning effort belong inline on every `agent()` call; `maxTurns`, `maxToolCalls`, and `maxToolOutputChars` may be omitted to use configured plugin defaults (`max_turns` 150, `max_tool_calls` 200, `max_tool_output_chars` 2000000, each clamped to its hard ceiling). Explicit inline values override those defaults; no budget is inherited from a role preset. Use the SAME phase titles in meta["phases"] as in phase() calls — titles are matched exactly; a phase() call with no matching meta entry just gets its own progress group.\n'
    '\n'
    'Script body hooks:\n'
    'Child budgets are hard ceilings, not telemetry. Always reserve a final model turn and tool call for structured output. Count inspect + execute + diagnose + report, not discovery alone.\n'
    'Starting points for broad work are a benchmark/environment loop at 25–40 turns and 50–100 tool calls, or broad multi-source synthesis at 16–30 turns and 20–50 tool calls. Keep benchmark evaluation separate from live probes. If a script has more than 3 phases, split phases through pipeline() or separate workflows. Evidence-only synthesizers should use toolsets=[] and no normal repository tools.\n'
    'A conservative low-literal-budget advisory may identify an agent label/index and limiting dimensions; launch continues; always increase only the limiting dimension or split via pipeline(), rather than raising every ceiling.\n'
    '- agent(prompt: str, opts: dict | None = None) -> Awaitable[Any] — spawn a subagent. Use `await agent(...)` for direct calls. Inside parallel()/pipeline() callbacks, return the awaitable; the helper will await it. Supported opts are `label`, `phase`, `schema`, `provider`, `model`, `isolation`, `agentType`, `toolsets`, `allowedTools`/`allowed_tools`, `disallowedTools`/`disallowed_tools`, `instructions`/`systemPrompt`, `reasoningEffort`, `maxTurns`, `maxToolCalls`, and `maxToolOutputChars`. `maxTurns` is optional: omitted values resolve from plugin config `max_turns` (default 150, clamped to 1..1000), while `maxToolCalls` and `maxToolOutputChars` are also optional and resolve from `max_tool_calls` (default 200, clamped to 1..10000) and `max_tool_output_chars` (default 2000000, clamped to 1..20000000). Explicit values override their configured defaults. All supplied budgets are exact integers and malformed or invalid explicit values fail before reservation and child launch. Current Bedrock and `codex_app_server` transports do not forward workflow reasoning effort, so those runtimes fail before child launch. `maxTurns` is an exact integer in 1..1000 and caps logical agent turns. `maxToolCalls` is an exact integer in 1..10000. `maxToolOutputChars` is an exact integer in 1..20000000. Resolved budgets participate in resume-cache identity. Provider retries within one logical turn do not consume additional `maxTurns`. Without schema, returns final text as a string. With schema (a JSON Schema), the subagent is forced to call the `structured_output` tool and agent() returns the validated object — no parsing needed. Returns None if the user skips the agent mid-run. opts["label"] overrides display label. opts["phase"] assigns the progress group. opts["model"] overrides the model for this call. opts["isolation"]: "worktree" runs in a fresh git worktree. opts["agentType"] may reference a `meta["agents"]` runtime preset or a library preset file (project/user/plugin). Explicit missing agentType errors by default; config `missing_agent_type_policy: fallback_warn` logs and falls back to `general-purpose`. Inline opts compose over the selected preset: inline instructions append, inline `toolsets` replace preset toolsets exactly (`toolsets: []` means no tools), `allowedTools` intersects preset allowlists (empty list means deny all except schema emission), and `disallowedTools` unions. Keep stable role text in `meta["agents"]`; put per-item facts in `prompt` to preserve resume/prompt-cache reuse.\n'
    '- codex(opts) -> Awaitable[dict] — run the native Codex CLI stage directly. Use `await codex({"mode": "discover", "workdir": "/absolute/repo", "contract": "..."})`; modes are `code`, `discover`, `debug`, and `verify`. `allowFiles` is required and must be a non-empty relative-path list for `code`, and is prohibited for read-only modes. Optional `timeout`, `label`, `phase`, and boolean `acceptExistingChanges` are supported; setting `acceptExistingChanges` to true opts into pre-existing changes only within `allowFiles`, while the default remains a clean entry. The stage uses the launcher defaults `gpt-5.6-luna` with `high` reasoning, returns its validated receipt, and participates in resume, progress, token accounting, `parallel()`, and `pipeline()`. Use direct parent Codex for one single bounded stage with no workflow orchestration. Do not route `agent() -> codex`, and codex stages never commit, push, rebase, review, or run tests.\n'
    ...
    '- workflow(name_or_ref: str | {"scriptPath": str}, args: Any = None) -> Awaitable[Any] — run another workflow inline as a sub-step and return whatever it returns. Call it with `await`. Pass a name to invoke a saved workflow (same registry as `{name: "..."}`), or `{"scriptPath": path}` to run a script file you wrote earlier. The child shares this run\'s concurrency cap, agent counter, abort signal, and token budget. The args param becomes the child\'s `args` global. Nesting is bounded by the configured max_nesting_depth (default 2, i.e. root plus 2 nested levels); a workflow() call past that depth raises. Run-wide caps (agent count, concurrency, token budget, deadline) are shared across all nesting levels, so deeper nesting never raises those ceilings. Raises on unknown name / unreadable scriptPath / child syntax error; catch Exception to handle gracefully.\n'
    '\n'
    ...
    '          {"label": "verify:" + f["file"], "phase": "Verify", "schema": VERDICT_SCHEMA, "provider": "openai-codex", "model": "gpt-5.6-luna", "reasoningEffort": "high", "maxTurns": 8, "maxToolCalls": 16, "maxToolOutputChars": 200000},\n'
    '      )\n'
    '      merged = dict(f)\n'
    ...
    '          d["prompt"],\n'
    '          {"label": "review:" + d["key"], "phase": "Review", "schema": FINDINGS_SCHEMA, "provider": "openai-codex", "model": "gpt-5.6-luna", "reasoningEffort": "high", "maxTurns": 8, "maxToolCalls": 16, "maxToolOutputChars": 200000},\n'
    '      ),\n'
    ...
    '      result = await agent("Find bugs in this codebase.", {"schema": BUGS_SCHEMA, "provider": "openai-codex", "model": "gpt-5.6-luna", "reasoningEffort": "high", "maxTurns": 8, "maxToolCalls": 16, "maxToolOutputChars": 200000})\n'
