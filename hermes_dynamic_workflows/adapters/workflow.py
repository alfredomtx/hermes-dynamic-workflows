...
_DESCRIPTION = (
    ...
    '\n'
    '  phase("Scan")\n'
    '  flaky = await agent("grep CI logs for retry markers", {"schema": FLAKY_SCHEMA, "provider": "openai-codex", "model": "gpt-5.6-luna", "reasoningEffort": "xhigh", "maxTurns": 8})\n'
    '  ...\n'
    '\n'
    'The `meta` dict must be a PURE LITERAL — no variables, function calls, dict unpacking, or f-string interpolation. Required fields: `name`, `description`. Optional: `whenToUse` (shown in the workflow list), `phases`, and `agents`. `meta["agents"]` defines workflow-local role presets for stable instructions and tool permissions only; provider, canonical `model`, and reasoning effort belong inline on every `agent()` call; `maxTurns` may be omitted to use the configured plugin default (`max_turns` 150). Explicit values override that default; no turn limit is inherited from a role preset. Use the SAME phase titles in meta["phases"] as in phase() calls — titles are matched exactly; a phase() call with no matching meta entry just gets its own progress group.\n'
    '\n'
    'Script body hooks:\n'
    'maxTurns is a hard logical-turn ceiling. Tool-call and tool-output counts are observational telemetry only. Always reserve a final model turn for structured output. Count inspect + execute + diagnose + report, not discovery alone.\n'
    ...
    '\n'
    'Scripts are restricted plain Python. The script body runs in an async context — use `await` directly. Available globals are intentionally narrow: `agent`, `start_agent`, `wait_agent`, `agent_status`, `continue_agent`, `fork_agent`, `stop_agent`, `parallel`, `pipeline`, `phase`, `log`, `args`, `budget`, `workflow`, `json`, `math`, safe built-ins, and common exception types. Imports, filesystem/process/network APIs, dunder traversal, dynamic eval/exec, class definitions, and dynamic call targets are rejected. Current time and randomness APIs are unavailable because they would break resume; pass timestamps in via `args`, stamp results after the workflow returns, and for randomness vary the agent prompt/label by index.\n'
    '\n'
    ...
    '          {"label": "verify:" + f["file"], "phase": "Verify", "schema": VERDICT_SCHEMA, "provider": "openai-codex", "model": "gpt-5.6-luna", "reasoningEffort": "xhigh", "maxTurns": 8},\n'
    '      )\n'
    '      merged = dict(f)\n'
    ...
    '          d["prompt"],\n'
    '          {"label": "review:" + d["key"], "phase": "Review", "schema": FINDINGS_SCHEMA, "provider": "openai-codex", "model": "gpt-5.6-luna", "reasoningEffort": "xhigh", "maxTurns": 8},\n'
    '      ),\n'
    ...
    '      result = await agent("Find bugs in this codebase.", {"schema": BUGS_SCHEMA, "provider": "openai-codex", "model": "gpt-5.6-luna", "reasoningEffort": "xhigh", "maxTurns": 8})\n'
