...
_DESCRIPTION = (
    ...
    '  ...\n'
    '\n'
    'The `meta` dict must be a PURE LITERAL — no variables, function calls, dict unpacking, or f-string interpolation. Required fields: `name`, `description`. Optional: `whenToUse` (shown in the workflow list), `phases`, and `agents`. `meta["agents"]` defines workflow-local role presets for stable instructions and tool permissions only; provider, canonical `model`, and reasoning effort belong inline on every `agent()` call; `maxToolCalls` and `maxToolOutputChars` are also required inline, while `maxTurns` may be omitted to use the configured `max_turns` default (150 by default, clamped to 1..1000). An explicit inline `maxTurns` overrides that default; no budget is inherited from a role preset. Use the SAME phase titles in meta["phases"] as in phase() calls — titles are matched exactly; a phase() call with no matching meta entry just gets its own progress group.\n'
    '\n'
    'Script body hooks:\n'
